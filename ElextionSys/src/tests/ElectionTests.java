...
public class ElectionTests {
	...
	void setUp() throws Exception {
		...
		//4 Partys
		elections.addNewParty("Halicud", WING.right);
		elections.addNewParty("Kahol-lavan", WING.center);
		elections.addNewParty("IL our home", WING.right);
		elections.addNewParty("Haavoda", WING.left);
	...
	public void checkBBox() {
		for (BBox bBox : elections.getAllBBox()) {
			if (bBox instanceof BBox) {
				...
				Scanner scan = new Scanner(elections.showRes());
			...
			if (bBox instanceof BBox) {
				for (Citizen citizen : bBox.getAllowedToVoteHere()) {
					if (citizen instanceof Citizen) {
						...
						assertTrue(citizen.getId() > 0);
						assertTrue(citizen.getBirthYear() > 1900);
